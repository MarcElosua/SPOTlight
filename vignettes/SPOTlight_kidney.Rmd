...
# Visualization
In the next section we show how to visualize the data and interpret `SPOTlight`'s
performance.
...
In the below table we can see how XXX **Elaborate on B cels etc...**
```{r}
library(DT)
sign <- basis(mod)
colnames(sign) <- paste0("Topic",1:ncol(sign))
datatable(sign, fillContainer = TRUE)
```
...
each spot. You can modify the colors as you would a standard `r CRANpkg("NMF")`.
```{r}
