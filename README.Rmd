...
We can extract the top marker genes from each cluster and select the unique ones to use as seeds for the model. This step is very important, cluster-marker genes are key for training the model! Feel free to add a row, representing a gene, in `cluster_markers_all` if you think it is an important marker. The use of the filters is the following:
- p_val_adj < 0.01 - we want to make sure the genes we are using are trully differentially expressed.
- avg_logFC > 1 - redundant with logfc.threshold = 1.
...
If the dataset is very large we want to down-sample it, both in terms of number of cells and number of genes, to train the model. To do this down-sampling we want to keep a representative amount of cells per cluster and the most important genes.
We will select first the genes of interest, to do so we will grab each cluster's markers plus the 3000 most variable genes.  
...
### Train model
We will set some parameters the we will use to train the model. K is the number of topics which we will assume to be the same as the number of clusters found by Seurat. With droplevels we make sure that there are no levels defined with no representation
```{r eval = FALSE}
...
Set seed words from top markers. Here we are setting the weights for each topic, the words that are weighted positively are those belonging to the list of top markers for a cluster. In the seed genes matrix each row represents a topic and each column represents a gene. To the LDA model we need to pass a matrix with k rows and ngene columns, where each cell has the weight of that gene for that topic. The weight we're assigning is the logFC 
```{r eval = FALSE}
# initialize matrix
...
### Spot Deconvolution
Create all possible combinations between 3-8 cells per spot.
We get the cell composition and the topic profile for each combination. We will compare the topic profiles of all these synthetic spots with the predicted topic profiles obtained from running the spatial spots through the LDA model.  The prediction process can be parallelized to speed it up.
...
Visualize them as a DotPlot for example:
```{r eval = FALSE}
round(clust_profiles, 4) %>% 
  ...
  mutate(
    ...
    `Cell identity` = factor(`Cell identity`, 
                             levels = 0:max(as.numeric(`Cell identity`))),
