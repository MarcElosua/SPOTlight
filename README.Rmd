...
The goal of SpatialDeconTest is to provide a tool that enables the deconvolution of cell types and cell type proportions present within each spot obtained from 10X's visium - spatial trancsiptomics- technology. It is based on finding topic profile signatures, by means of a Latent Dirichlet Allocation model, for each cell type and finding which combination fits best the spot we want to deconvolute.
...
- **p_val_adj** < 0.01 - we want to make sure the genes we are using are trully differentially expressed.
