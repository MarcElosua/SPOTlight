...
graph_col_df <- data.frame(
    value = as.character(grad_edge),
...
```{r eval = FALSE}
ct_topic_profiles <- topic_profile_per_cluster_nmf(
    h = h,
    train_cell_clust = nmf_mod_ls[[2]]
)

decon_mtrx <- mixture_deconvolution_nmf(
    nmf_mod = nmf_mod,
