...
cortex_sc <- readRDS("../spatial_deconvolution/data/MusMusculus/allen_cortex.rds")
```

``` r
cortex_sc <- Seurat::SCTransform(cortex_sc)
...
saveRDS(object = cortex_sc,
        file = "sample_data/cortex_scRNAseq_processed.RDS")
```

Read directly the processed data

``` r
cortex_sc <- readRDS(file = "sample_data/cortex_scRNAseq_processed.RDS")
```
...
``` r
anterior <- readRDS(file = "sample_data/brain_anterior_ST_processed.RDS")
```
