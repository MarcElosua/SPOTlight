...
test_synthetic_performance <- function(test_spots_metadata_mtrx, spot_composition_mtrx){
  ...
  tp=0;tn=0;fp=0;fn=0
  for (i in 1:nrow(test_spots_metadata_mtrx)) {
  ...
  cat(sprintf('The following summary statistics are obtained:
              ...
              F1 score: %s,
              JSD quantiles: %s[%s-%s]',
              accuracy, sensitivity, specificity, precision, recall, F1, quants_JSD[[2]], quants_JSD[[1]], quants_JSD[[3]]), sep = '\n')

  cat('raw statistics are returned in the list - TP, TN, FP, FN, JSD quantiles',sep = '\n')
