...
test_spot_fun <- function(se_obj, clust_vr, n=10000, verbose=TRUE){
  
  # Check variables
  if(is(se_obj)!="Seurat") {stop("ERROR: se_obj must be a Seurat object!")}
  if(!is.character(clust_vr)){stop("ERROR: clust_vr must be a character string!")}
  if(!is.integer(n)){stop("ERROR: n must be an integer!")}
  if(!is.logical(verbose)){stop("ERROR: verbose must be a logical object!")}
  
  cat("Loading packages...", sep="\n")
  suppressMessages(require(library(DropletUtils))) # For the downsampling
  suppressMessages(require(library(dtplyr))) # To use dplyr commands with DT speed
