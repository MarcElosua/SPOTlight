...
cut_markers2 <- function(markers,
  ...
  min_pval <-  unique(sort(markers$p_val_adj))[2]
  tmp_markers <- markers %>%
    dplyr::arrange(cluster, p_val, avg_logFC) %>%
    dplyr::mutate(
      ...
      # weight = scale(x = avg_logFC, center = F, scale = max(avg_logFC)),
      # pval_inv = dplyr::if_else(p_val_adj == 0,
      #                           1 / (p_val_adj + .Machine$double.xmin),
      #                           1 / p_val_adj),
      # p_val_adj_z = scale(x = pval_inv, center = FALSE, scale = max(p_val_adj))
      # p_val_adj_z = scale(x = pval_inv, center = FALSE, scale = TRUE)
      weight = 1 - p_val
      ) %>%
