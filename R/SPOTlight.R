...
SPOTlight <- function(
    ...
    # train NMF model
    mod_ls <- trainNMF(x, y, groups, mgs, n_top, gene_id, 
                       group_id, weight_id, hvg, scale, verbose, 
                       L1_nmf, L2_nmf, tol, maxit, threads, assay_sc, 
                       assay_sp, slot_sp, ...)

    # perform deconvolution
    res <- runDeconvolution(x, mod_ls$mod, mod_ls$topic, scale, min_prop, 
                            verbose, assay_sp, slot_sp, L1_nnls, L2_nnls, threads, ...)
